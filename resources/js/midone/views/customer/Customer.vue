        ...
        <DataList :title="t('views.customer.table.title')" :data="customerList" v-on:createNew="createNew" v-on:dataListChange="onDataListChange" :enableSearch="true">
           <template v-slot:table="tableProps">
                <table class="table table-report -mt-2">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ t('views.customer.table.cols.company') }}</th>
                            ...
                            <tr :class="{'intro-x':true, 'hidden transition-all': expandDetail !== itemIdx}">
                                <td colspan="6">
                                    <div class="flex flex-row">
                                        <div class="ml-5 w-48 text-right pr-5">{{ t('views.customer.fields.company_id') }}</div>
        ...
        <div class="loader-container">
            <VeeForm id="customerForm" class="p-5" @submit="onSubmit" @invalid-submit="invalidSubmit" v-slot="{ handleReset, errors }">
                <div class="p-5">
                    <!-- #region Company -->
                    <div class="mb-3">
                        <label class="form-label" for="inputCompany_id">{{ t('views.customer.fields.company_id') }}</label>
                        <VeeField as="select" id="company_id" name="company_id" :class="{'form-control form-select':true, 'border-danger': errors['company_id']}" v-model="customer.company.hId" :label="t('views.customer.fields.company_id')" rules="required" @blur="reValidate(errors)">
                    ...
                    <!-- #region Addresss -->
                    <div class="mb-3">
                        <label for="inputAddress" class="form-label">{{ t('views.customer.fields.address') }}</label>
                        <textarea id="inputAddress" name="address" type="text" class="form-control" :placeholder="t('views.customer.fields.address')" v-model="customer.address" rows="3"></textarea>
                    </div>
                    <!-- #endregion -->

                    <!-- #region City -->
                    <div class="mb-3">
                        <label for="inputCity" class="form-label">{{ t('views.customer.fields.city') }}</label>
                        <input id="inputCity" name="city" type="text" class="form-control" :placeholder="t('views.customer.fields.city')" v-model="customer.city" />
                    </div>
                    <!--  #endregion -->

                    <!-- #region Contact -->
                    <div class="mb-3">
                        <label for="inputContact" class="form-label">{{ t('views.customer.fields.contact') }}</label>
                        <input id="inputContact" name="contact" type="text" class="form-control" :placeholder="t('views.customer.fields.contact')" v-model="customer.contact" />
                    </div>
                    <!-- #endregion -->
...
const getAllCustomers = (args) => {
    ...
    axios.get(route('api.get.db.company.customer.read', { "companyId": companyId, "page": args.page, "perPage": args.pageSize, "search": args.search })).then(response => {
...
const onSubmit = (values, actions) => {
    ...
    if (mode.value === 'create') {
        axios.post(route('api.post.db.company.customer.save'), formData).then(response => {
            backToList();
        }).catch(e => {
        ...
        axios.post(route('api.post.db.company.customer.edit', customer.value.hId), formData).then(response => {
            actions.resetForm();
            backToList();
        }).catch(e => {
            handleError(e, actions);
...
const createNew = () => {
    ...
    customer.value.company = _.find(companyDDL.value, { 'hId': selectedUserCompany.value });
}
...
const confirmDelete = () => {
    ...
    axios.post(route('api.post.db.company.customer.delete', deleteId.value)).then(response => {
        backToList();
    }).catch(e => {
