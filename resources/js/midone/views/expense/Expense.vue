        ...
        <DataList :title="t('views.expense.table.title')" :data="expenseList" v-on:createNew="createNew" v-on:dataListChange="onDataListChange" :enableSearch="true">
           <template v-slot:table="tableProps">
                <table class="table table-report -mt-2">
                    <thead>
                        <tr>
                            <th class="whitespace-nowrap">{{ t('views.expense.table.cols.company') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.expense.table.cols.code') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.expense.table.cols.name') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.expense.table.cols.remarks') }}</th>
                            <th class="whitespace-nowrap">{{ t('views.expense.table.cols.status') }}</th>
        ...
        <div class="loader-container">
            <VeeForm id="expenseForm" class="p-5" @submit="onSubmit" @invalid-submit="invalidSubmit" v-slot="{ handleReset, errors }">
                <div class="p-5">
                    <!-- #region Company -->
                    <div class="mb-3">
                        <label class="form-label" for="inputCompany_id">{{ t('views.expense.fields.company_id') }}</label>
                        <VeeField as="select" id="company_id" name="company_id" :class="{'form-control form-select':true, 'border-danger': errors['company_id']}" v-model="expense.company.hId" :label="t('views.expense.fields.company_id')" rules="required" @blur="reValidate(errors)">
                    ...
                    <!-- #region Name -->
                    <div class="mb-3">
                        <label for="inputName" class="form-label">{{ t('views.expense.fields.name') }}</label>
                        <VeeField id="inputName" name="name" type="text" :class="{'form-control':true, 'border-danger': errors['name']}" :placeholder="t('views.expense.fields.name')" :label="t('views.expense.fields.name')" rules="required" @blur="reValidate(errors)" v-model="expense.name" />
                    ...
                    <!-- #region Addresss -->
                    <div class="mb-3">
                        <label for="inputAddress" class="form-label">{{ t('views.expense.fields.address') }}</label>
                        <textarea id="inputAddress" name="address" type="text" class="form-control" :placeholder="t('views.expense.fields.address')" v-model="expense.address" rows="3"></textarea>
                    </div>
                    <!-- #endregion -->

                    <!-- #region City -->
                    <div class="mb-3">
                        <label for="inputCity" class="form-label">{{ t('views.expense.fields.city') }}</label>
                        <input id="inputCity" name="city" type="text" class="form-control" :placeholder="t('views.expense.fields.city')" v-model="expense.city" />
                    </div>
                    <!--  #endregion -->

                    <!-- #region Contact -->
                    <div class="mb-3">
                        <label for="inputContact" class="form-label">{{ t('views.expense.fields.contact') }}</label>
                        <input id="inputContact" name="contact" type="text" class="form-control" :placeholder="t('views.expense.fields.contact')" v-model="expense.contact" />
                    </div>
                    <!-- #endregion -->
                    ...
                    <!-- #region Status -->
                    <div class="mb-3">
                        <label for="status" class="form-label">{{ t('views.expense.fields.status') }}</label>
                        <VeeField as="select" id="status" name="status" :class="{'form-control form-select':true, 'border-danger': errors['status']}" v-model="expense.status" rules="required" @blur="reValidate(errors)">
                            <option value="">{{ t('components.dropdown.placeholder') }}</option>
...
const getAllExpenses = (args) => {
    ...
    axios.get(route('api.get.db.company.expense.read', { "companyId": companyId, "page": args.page, "perPage": args.pageSize, "search": args.search })).then(response => {
...
const onSubmit = (values, actions) => {
    ...
    if (mode.value === 'create') {
        axios.post(route('api.post.db.company.expense.save'), formData).then(response => {
            backToList();
        }).catch(e => {
        ...
        axios.post(route('api.post.db.company.expense.edit', expense.value.hId), formData).then(response => {
            actions.resetForm();
            backToList();
        }).catch(e => {
            handleError(e, actions);
...
const createNew = () => {
    ...
    expense.value.company = _.find(companyDDL.value, { 'hId': selectedUserCompany.value });
}
...
const confirmDelete = () => {
    ...
    axios.post(route('api.post.db.company.expense.delete', deleteId.value)).then(response => {
        backToList();
    }).catch(e => {
