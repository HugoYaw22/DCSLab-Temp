        ...
        <div class="loader-container">
            <VeeForm id="capitalForm" class="p-5" @submit="onSubmit" @invalid-submit="invalidSubmit" v-slot="{ handleReset, errors }">
                <div class="p-5">                    
                    <!-- #region Code -->
                    <div class="mb-3">
                        <label for="inputCode" class="form-label">{{ t('views.capital.fields.code') }}</label>
                        <div class="flex items-center">
                            <VeeField id="inputCode" name="code" type="text" :class="{'form-control':true, 'border-danger': errors['code']}" :placeholder="t('views.capital.fields.code')" :label="t('views.capital.fields.code')" rules="required" @blur="reValidate(errors)" v-model="capital.code" :readonly="capital.code === '[AUTO]'" />
                    ...
                    <!-- #region Name -->
                    <div class="mb-3">
                        <label for="inputName" class="form-label">{{ t('views.capital.fields.name') }}</label>
                        <VeeField id="inputName" name="name" type="text" :class="{'form-control':true, 'border-danger': errors['name']}" :placeholder="t('views.capital.fields.name')" :label="t('views.capital.fields.name')" rules="required" @blur="reValidate(errors)" v-model="capital.name" />
                    ...
                    <!-- #region Addresss -->
                    <div class="mb-3">
                        <label for="inputAddress" class="form-label">{{ t('views.capital.fields.address') }}</label>
                        <textarea id="inputAddress" name="address" type="text" class="form-control" :placeholder="t('views.capital.fields.address')" v-model="capital.address" rows="3"></textarea>
                    </div>
                    <!-- #endregion -->
                    ...
                    <!-- #region Contact -->
                    <div class="mb-3">
                        <label for="inputContact" class="form-label">{{ t('views.capital.fields.contact') }}</label>
                        <input id="inputContact" name="contact" type="text" class="form-control" :placeholder="t('views.capital.fields.contact')" v-model="capital.contact" />
                    </div>
                    <!-- #endregion -->
                    ...
                    <!-- #region Status -->
                    <div class="mb-3">
                        <label for="status" class="form-label">{{ t('views.capital.fields.status') }}</label>
                        <VeeField as="select" id="status" name="status" :class="{'form-control form-select':true, 'border-danger': errors['status']}" v-model="capital.status" rules="required" @blur="reValidate(errors)">
...
const getAllCapitals = (args) => {
    ...
    axios.get(route('api.get.db.company.capital.read', { "companyId": companyId, "page": args.page, "perPage": args.pageSize, "search": args.search })).then(response => {
...
const onSubmit = (values, actions) => {
    ...
    if (mode.value === 'create') {
        axios.post(route('api.post.db.company.capital.save'), formData).then(response => {
            backToList();
        }).catch(e => {
        ...
        axios.post(route('api.post.db.company.capital.edit', capital.value.hId), formData).then(response => {
            actions.resetForm();
            backToList();
        }).catch(e => {
            handleError(e, actions);
...
const createNew = () => {
    ...
    capital.value.company = _.find(companyDDL.value, { 'hId': selectedUserCompany.value });
}
...
const confirmDelete = () => {
    ...
    axios.post(route('api.post.db.company.capital.delete', deleteId.value)).then(response => {
        backToList();
    }).catch(e => {
